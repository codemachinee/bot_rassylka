...
class model_buttons: # класс формирования клавиатур
    ...
    async def rasylka_buttons(self):
        ...
        but1 = types.KeyboardButton(text='база1')
        but2 = types.KeyboardButton(text='база2')
        but3 = types.KeyboardButton(text='база3')
...
class clients_base:  # класс базы данных
    ...
    def __init__(self, bot, message, message_id, auto_model, base=None):
        self.bot = bot
        self.message = message
        ...
        sh = gc.open('base')
        self.worksheet = sh.worksheet('база1')  # выбор листа 'общая база клиентов' таблицы
        self.worksheet2 = sh.worksheet('база2')
        self.worksheet3 = sh.worksheet('база3')
    ...
    async def rasylka_v_bazu(self):  # функция рассылки постов в базы
        ...
        if self.base == 'база1':
            ...
            await self.bot.edit_message_text(f'Босс, рассылка в базу1 выполнена ✅', self.message.chat.id,
            ...
            for i in range(1, len(self.worksheet.col_values(1))):
                try:
                    ...
                    await self.bot.send_message(self.message.chat.id, f'Босс, похоже @{self.worksheet2.col_values(2)[i]} '
                                                ...
                                                reply_markup=kb5)
            await self.bot.edit_message_text(f'Босс, рассылка в базу2 выполнена ✅', self.message.chat.id,
                                             message_id=self.message.message_id + 1)
        if self.base == 'база3':
            for i in range(1, len(self.worksheet.col_values(1))):
                try:
                    await self.bot.copy_message(self.worksheet3.col_values(1)[i], self.message.chat.id,
                                                self.message_id, reply_markup=kb5)
                    ...
                    await self.bot.send_message(self.message.chat.id, f'Босс, похоже @{self.worksheet2.col_values(2)[i]} '
                                                                      f'заблочил меня. Рассылка невозможна',
                                                reply_markup=kb5)
            await self.bot.edit_message_text(f'Босс, рассылка в базу3 выполнена ✅', self.message.chat.id,
                                             message_id=self.message.message_id + 1)
